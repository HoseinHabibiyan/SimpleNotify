...
{
    record AddOrderNotification(string OrderNumber,int Quantity) : INotification<AddOrderNotification>;
    record AddOrderNotificationHandler(TextWriter Writer) : INotificationHandler<AddOrderNotification>
    {
    ...
    public async Task When_AddOrderNotification_Is_Published_Should_Invoke_Handler_As_Expected()
    {
        ...
        SimpleNotifySender simpleNotifySender = new();
        
        AddOrderNotification orderNotification = new (Guid.NewGuid().ToString(), new Random().Next());
        
        ServiceCollection serviceCollection = new();
        serviceCollection.AddSimpleNotify(typeof(SimpleNotifySender).Assembly);
